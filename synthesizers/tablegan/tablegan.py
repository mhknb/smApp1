...
def main():
    ...
    dset = Dataset(
        ...
        output_directory= args.exp_name,
    )
    ...
    if args.train:
        ...
        model_save_path = os.path.join(save_dir, "tablegan_model.pkl")
    ...
    if args.evaluate:
        ...
        data_transformer = DataTransformer(
            discrete_encode= args.numerical_preprocess ,
            numerical_preprocess=args.discrete_preprocess,
        ...
        # table_evaluator(fake_data, train_data)
        # model.table_evaluator_m
    ...
    if args.sample:
        ...
        mkdir(fake_dir)
        os.makedirs(os.path.join(fake_dir, "fakedata"), exist_ok=True)
        os.makedirs(os.path.join(fake_dir, "traindata"), exist_ok=True)
        ...
        for i in range(args.eval_retries):
            ...
            fake_data.sample(args.sample_size).to_csv(
                os.path.join(fake_dir,"fakedata", f"fakedata_{i}.csv"), index=None
            )
            train_data.sample(args.sample_size).to_csv(
                os.path.join(fake_dir,"traindata", f"train_{i}.csv"), index=None
            )
            
            # table_evaluator_m( args.target_name,train_data,fake_data,os.path.join(fake_dir,f"tableEvaluator{i}"))
...
def parse_arguments():
    ...
    ## Dataset
    parser.add_argument("--numerical_preprocess", type=str, default="minmax", choices=["bayesian", "standard", "minmax", "none"], help="preprocessing scheme to use for numerical columns")
